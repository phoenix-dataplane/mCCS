...
class Primitives<
  ...
  static constexpr int RoleInput = 0x01,
                       ...
                       // DirectRead = 0x400,
                       ThreadsSynced = 0x800;
  // the class members are per GPU-thread
  const int tid;
  int nthreads;
  int nworkers;
  const int stepSize;
  ...
  inline __device__ bool checkAbort(int &spins) {
    spins++;
    if (!(flags & Aborted) && spins == MCCS_SPINS_BEFORE_CHECK_ABORT) {
      flags |= *mccsShmem.comm.abortFlag ? Aborted : 0;
      spins = 0;
    }
  ...
  __device__ __forceinline__ void waitPeer(intptr_t dstIx, intptr_t remoteIx, int offset, int nelts) {
    ...
    if ((flags & (Recv*RoleWaitRecv)) ||
      ...
      int spins = 0;
      while (connStepCache + (isSendNotRecv ? MCCS_BUFFER_SLOTS : 0) < step + StepPerSlice) {
        connStepCache = *connStepPtr;
        if (checkAbort(spins)) break;
        //if (spins == 0) printf("r=%d b=%d t=%d SPUN OUT got=%d want=%d\n", mccsShmem.comm.rank, blockIdx.x, threadIdx.x, int(connStepCache + (isSendNotRecv ? NCCL_STEPS : 0)), int(step+StepPerSlice));
      }
    }

    if (flags & (Recv*RoleWaitRecv | Send*RoleWaitSend)) {
      if (isSendNotRecv && (flags & SizesFifoEnabled))
        connSizesFifoPtr[step%MCCS_BUFFER_SLOTS] = nelts*sizeof(T);

      void **ptrs = isSendNotRecv ? (mccsShmem.groups[group].dsts + Dst)
                                  : (mccsShmem.groups[group].srcs + Src);
      if (flags & OffsFifoEnabled)
        ptrs[index] = connEltsFifo + loadInt(connOffsFifoPtr + (step%MCCS_BUFFER_SLOTS))/sizeof(T);
        ...
        //   ptrs[index] = nullptr;
        // } else {
        //   // set src or dst pointer
        //   ptrs[index] = connEltsFifo + (step%NCCL_STEPS)*stepSize;
        // }
        ptrs[index] = connEltsFifo + (step%MCCS_BUFFER_SLOTS)*stepSize;
...
//               ((char*)(mccsShmem.groups[group].srcs[0])-(char*)userBuff));
          ...
          if (Send) {
            ...
            ReduceOrCopyMulti<Unroll, RedOp, T, 1, 1, 1, (1-Send)+MaxSend, 0>
              (tid, nworkers, nullptr, false,
               1, (T const**)mccsShmem.groups[group].srcs,
               fan.nsend(), (T**)mccsShmem.groups[group].dsts+1,
               sliceSize);
          }
        } else if (DirectSend && !DirectRecv && SrcBuf != Input && mccsShmem.groups[group].dsts[Dst] == nullptr) {
          // For broadcast in CollNet to do empty send
          ReduceOrCopyMulti<Unroll, RedOp, T, 1, 1, 1, 1, 0>
            (tid, nworkers, mccsShmem.redOpArgs, postOp,
             Recv, (T const**)mccsShmem.groups[group].srcs,
             Dst, (T**)mccsShmem.groups[group].dsts,
             sliceSize);
        } else {
          // constexpr int PreOpN = SrcBuf != Input ? 0 :
          //                        DirectRecv*MaxRecv == NCCL_MAX_DIRECT_ARITY ? (1+NCCL_MAX_DIRECT_ARITY) : 1;
          constexpr int PreOpN = SrcBuf != Input ? 0 : 1;
          ReduceOrCopyMulti<Unroll, RedOp, T, Recv+Src, Recv*MaxRecv+Src, Send+Dst, Send*MaxSend+Dst, PreOpN>
            (tid, nworkers, mccsShmem.redOpArgs, postOp,
             Recv*fan.nrecv()+Src, (T const**)mccsShmem.groups[group].srcs,
             Send*fan.nsend()+Dst, (T**)mccsShmem.groups[group].dsts,
             sliceSize);
        }
  ...
  ScatterGatherOp(intptr_t inpIx, intptr_t outIx, int totalElem, int peerElem, int skip, int shift, bool postOp) {
    ...
    for (int slice=0; slice<SlicePerChunk; ++slice) {
      // send one slice of each part i to peer i
      ...
      if (tid < nworkers) {
        if (Send) {
          ...
          // offset < peerElem, current offset within each peer's part
          if (flags & RoleInput) mccsShmem.groups[group].srcs[0] = userBuff + inpIx + offset;
          if (tid == 0) mccsShmem.groups[group].totalSendSize[slice] = 0; // Skip the threadfence
          ...
          for (int j=0; j<fan.nsend(); j++) {
            ...
            const T* src0 = (T*)mccsShmem.groups[group].srcs[0] + peerOffset;
            // the last part (send to dsts[fan.nsend()-1]) may exceed totalElem 
            // the remaining totalElem-peerOffset could be less than realSize
            int realPeerSize = min(realSize, totalElem-peerOffset);
            if (realPeerSize > 0 && mccsShmem.groups[group].dsts[i] != nullptr) {
              ReduceOrCopyMulti<Unroll, RedOp, T, 1, 1, 1, 1, PreOpN>(tid, nworkers, mccsShmem.redOpArgs, false, 1, &src0, 1, (T**)mccsShmem.groups[group].dsts+i, realPeerSize);
              // Mark for threadfence at the end
              if (tid == 0) mccsShmem.groups[group].totalSendSize[slice] += realPeerSize;
            }
          }
        } else if (Recv) {
          if (flags & RoleOutput) mccsShmem.groups[group].dsts[0] = userBuff + outIx + offset;
          ...
          if (DirectRecv && mccsShmem.groups[group].srcs[0] == mccsShmem.groups[group].dsts[0]) {
            ...
            for (int j=0; j<fan.nrecv(); j++) {
              ...
              if (skip >= 0 && i >= skip) peerOffset += peerElem;
              T* dst0 = (T*)mccsShmem.groups[group].dsts[0] + peerOffset;
              int realPeerSize = min(realSize, totalElem-peerOffset);
              if (realPeerSize > 0) ReduceOrCopyMulti<Unroll, RedOp, T, 1, 1, 1, 1, 0>(tid, nworkers, mccsShmem.redOpArgs, postOp, 1, (const T**)mccsShmem.groups[group].srcs+i, 1, &dst0, realPeerSize);
            }
          }
        }
      }
      ...
      if (Send && (flags & RolePostSend) && mccsShmem.groups[group].totalSendSize[slice] > 0 && index == 0)
  ...
  __device__ __forceinline__ void loadRecvConn(mccsDevChannelPeer *peer, int connIndex, struct mccsDevWorkElem* e) {
    if (flags & (RoleWaitRecv|RolePostRecv)) {
      ...
      if (flags & RoleWaitRecv) {
        mccsShmem.groups[group].recvConns[index] = conn; // WaitRecv role saves since that's who needs it in setDataPtrs()
        ...
        connEltsFifo = (T*)conn->buffs[MCCS_PROTO_SIMPLE];
      }
    }
  }

  __device__ __forceinline__ void loadSendConn(mccsDevChannelPeer *peer, int connIndex, struct mccsDevWorkElem* e) {
    if (flags & (RoleWaitSend|RolePostSend)) {
      ...
      if (flags & RoleWaitSend) {
        mccsShmem.groups[group].sendConns[index] = conn; // WaitSend role saves since that's who needs it in setDataPtrs()
        connStepPtr = conn->head;
        connStepCache = *connStepPtr;
        flags |= (conn->offsFifo != nullptr) ? OffsFifoEnabled : 0;
        if (flags & OffsFifoEnabled)
        ...
        if (conn->sizesFifo != nullptr) {
          ...
          // sender should modify this to indicate the size of each buffer slot?
 ...
 public:
  __device__ Primitives(
      ...
      void const *inputBuf, void *outputBuf, uint64_t redOpArg, uint32_t group=0, struct mccsDevWorkElem* e = nullptr
    ):
    tid(tid),
    stepSize(mccsShmem.comm.buffSizes[MCCS_PROTO_SIMPLE]/MCCS_BUFFER_SLOTS/sizeof(T)) {
    ...
    loadRecvConn(&mccsShmem.channel.peers[peer], connIndex, e);
    loadSendConn(&mccsShmem.channel.peers[peer], connIndex, e);

    setDataPtrs(inputBuf, outputBuf, redOpArg, (struct mccsDevWorkElemReg*)e);
  }

  __device__ ~Primitives() {
    // Ensure mccsShmem.groups[].send/recvConns are available
    if (!(flags & ThreadsSynced))
      barrier();
    // Save steps for the next operation
    if (flags & (RolePostSend|RolePostRecv)) {
      auto *conns = (flags & RolePostSend) ? mccsShmem.groups[group].sendConns : mccsShmem.groups[group].recvConns;
      conns[index]->step = step;
    }
    // Make sure all threads are done writing back conn->step and done using
    // mccsShmem.groups[group]
    barrier();
  }

  __device__ void setDataPtrs(void const *inputBuf, void *outputBuf, uint64_t redOpArg, struct mccsDevWorkElemReg* e) {
    if (flags & RoleInput) {
      userBuff = (T*)inputBuf;
      // redOpArgs for local input buffer
      mccsShmem.redOpArgs[0] = redOpArg;  // scaler for local input
    }
    ...
    //   int spins = 0;
    //   void *volatile *slot = mccsShmem.groups[group].recvConns[index]->ptrExchange;
    ...
    //   int spins = 0;
    //   void *volatile *slot = mccsShmem.groups[group].sendConns[index]->ptrExchange;
    //   void *ptr;
    //   while (true) {
    //     ptr = *slot;
    //     if (ptr != nullptr || checkAbort(spins)) break;
    //   }
    //   // decode the poineter encoded the way above
    ...
    //   int spins = 0;
    //   void *volatile *slot = mccsShmem.groups[group].sendConns[index]->ptrExchange;
    //   volatile uint64_t* argSlot0 = mccsShmem.groups[group].sendConns[index]->redOpArgExchange;
    //   volatile uint64_t* argSlot1 = mccsShmem.groups[group].sendConns[index]->redOpArgExchange+1;
    //   // Wait for consumer to consume previous value before trampling it.
    ...
    //   int spins = 0;
    //   void *volatile *slot = mccsShmem.groups[group].recvConns[index]->ptrExchange;
    //   volatile uint64_t* argSlot0 = mccsShmem.groups[group].recvConns[index]->redOpArgExchange;
    //   volatile uint64_t* argSlot1 = mccsShmem.groups[group].recvConns[index]->redOpArgExchange+1;
    //   void *ptr;
    //   while (true) {
    //     ptr = *slot;
    //     if (ptr != nullptr || checkAbort(spins)) break;
    ...
    //     }
    //     // receiver, redOpArgs for remote inputs
    //     mccsShmem.redOpArgs[1+index] = ((arg1 & 0xffffffff)<<32) | (arg0 & 0xffffffff);
    //   }
