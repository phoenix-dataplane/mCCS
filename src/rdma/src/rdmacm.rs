...
impl<'res> CmId<'res> {
    ...
    pub fn reg_msgs<'a>(&self, buf: &'a [u8]) -> io::Result<MemoryRegion<'a>> {
        ...
        let addr = buf.as_ptr();
        let length = buf.len();
        let mr = unsafe { ffi::rdma_reg_msgs_real(id, addr as *mut _, length) };
        if mr.is_null() {
            return Err(io::Error::last_os_error());
        }
    ...
    pub unsafe fn post_send<'a>(
        ...
        let rc = ffi::rdma_post_send_real(id, context, addr as *mut _, length, mr, flags.0 as _);
        if rc != 0 {
            return Err(io::Error::last_os_error());
        }
        Ok(())
    ...
    pub unsafe fn post_recv<'a>(
        ...
        let rc = ffi::rdma_post_recv_real(id, context, addr as *mut _, length, mr);
        if rc != 0 {
            return Err(io::Error::last_os_error());
        }
        Ok(())
