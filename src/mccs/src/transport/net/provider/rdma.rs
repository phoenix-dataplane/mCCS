...
#[derive(Debug, Error)]
pub enum IbError {
    #[error("ibverbs API failed with errno {0}")]
    IbVerbs(#[from] Errno),
...
pub struct IbMrHandle(MemoryRegionRegister);
pub struct IbRequestId(usize);

pub fn ib_get_request(verbs: &mut IbVerbs<'_>) -> Option<usize> {
...
pub fn ib_multi_send(
    ...
    for qp in comm.qps.iter_mut() {
        ...
        unsafe { 
            ...
            ibv_post_check!(send_fn(qp_ptr, comm.wrs.as_mut_ptr(), &mut bad_wr));
        }
